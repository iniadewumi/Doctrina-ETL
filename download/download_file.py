...
params['now_day_02'] = f"{now.day:02d}"
params['now_month'] = str(now.month)
params['now_month_02'] = f"{now.month:02d}"
params['now_month_text'] = now.strftime('%B')
params['now_year'] = str(now.year)

def parameterise_input(input):
    for i in params: input=input.replace("${"+f'{i}'+"}",params[i])
    return input
